...
const uploadToCloudinary =  (fileBuffer: Buffer): Promise<{secure_url: string}> => {
 ...
 // Get owner's restararant
// Ger /api/owner/restaurant
...
// Create owner's restararant(submitted to pending)
...
export const updateOwnerRestaurant = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        if(availableSlots){
            restaurant.avilableSlots = typeof availableSlots === "string" ? availableSlots.split(",").map((s)=> s.trim()): availableSlots;
        }
...
// Get bookings for owner's restararant
...
export const updateOwnerBookingStatus = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        const { status } = req.body;
        if(!status || !["confirmed", "cancelled", "completed"].includes(status)){
            res.status(400).json({ message: "Please enter a valid bookingstatus " });
        ...
        if(!restaurant || restaurant.owner.toString() !== req.user?._id.toString()){
            res.status(404).json({ message: "No authorized to manage this booking" });
            return;
        }
