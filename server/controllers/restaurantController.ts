...
// Get all restruent with search and filter
// Get /api/restaurents
export const getRestaurents = async (req: Request, res: Response): Promise<void>=> {
    try {
        ...
        if(location){
            queryObj.location = {$regex: location as string, $option: "i"};
        }
        // sorting
        ...
        const resturant = await Restaurant.find(queryObj).sort(sortOption);
        res.json(Restaurant)
...
// Get all featured and exclusive restuarants
// Get /api/restaurents/featured
export const getFeaturedRestaurents = async (req: Request, res: Response): Promise<void>=> {
    try {
...
// Get /api/restaurents/: slug
export const getRestaurentBySlug = async (req: Request, res: Response): Promise<void>=> {
    try {
...
// Get dynamic seat avilability for slots
// Get /api/restaurents/: id/avilability
export const getRestaurentAvailability = async (req: Request, res: Response): Promise<void>=> {
    try {
       ...
       if(!date){
        ...
        //Get all active booking on this date for the restaurent
        const bookings = await Booking.find({
            restaurant: restaurant._id,
            date: bookingDate,
            status: "confirmed",
        })
        //Map slots to avilable capacities
        const avilability = restaurant.avilableSlots.map(( slot)=>{
